...
export default function AdminMR() {
  ...
  useEffect(() => {
    const fetchInput = () => {
      ...
      const unsubscribe = onSnapshot(collection(db, 'market-research'), (querySnapshot) => {
  ...
  function updateSelect() {
    try {
      const dropdown = document.getElementById("mr-dropdown");
      dropdown.innerHTML = "";
      posts.forEach(post => {
        const newOption = document.createElement("option");
        newOption.value = post.key; // Use a unique identifier for each option
        newOption.text = post.businessName; // Display name for users
